...
const SOURCES: [MediaType, string, number][] = [
...
async function fetchTmdbPage(endpoint: string, page: number): Promise<{ results: TmdbListItem[] }> {
  ...
  return res.json() as Promise<{ results: TmdbListItem[] }>;
}

function toIndexItem(raw: TmdbListItem, mediaType: MediaType): IndexItem {
  return {
    tmdbId: raw.id,
    mediaType,
...
async function postBatch(items: IndexItem[]): Promise<{ indexed?: number }> {
  const res = await fetch(`${WORKER_BASE_URL}/admin/index`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Admin-Key": ADMIN_KEY! },
    body: JSON.stringify({ items }),
  });
  const data = (await res.json().catch(() => ({}))) as { indexed?: number };
...
async function main(): Promise<void> {
  ...
  const seen = new Map<string, IndexItem>();
