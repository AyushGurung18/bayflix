...
interface RatingsBadgesProps {
  ratings: RatingsResult | null | undefined;
  tmdbScore?: number;
}

export default function RatingsBadges({ ratings, tmdbScore }: RatingsBadgesProps) {
  const tmdbBadge = tmdbScore && tmdbScore > 0 && (
    <Badge label="TMDB" value={`${tmdbScore.toFixed(1)}/10`} accent={TMDB_GREEN} />
  );
...
function Badge({ label, value, accent, hint }: BadgeProps) {
  return (
    <span
      title={hint}
