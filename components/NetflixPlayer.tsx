...
type SettingsPanel = false | "quality" | "audio" | "subtitles";
type NetworkCondition = "good" | "medium" | "poor";

// hls.js's MediaPlaylist type is large and only a couple of fields are ever
// read here — a small local shape is clearer than fighting the full import.
...
export default function NetflixPlayer({
  ...
  const [showSettings, setShowSettings] = useState<SettingsPanel>(false);
  ...
  const findLevelByQuality = useCallback((levels: Level[] | undefined, targetQuality: string) => {
  ...
  const initializeVideo = useCallback(
    (quality: string, resumeTime = 0) => {
      ...
      hls.on(Hls.Events.AUDIO_TRACKS_UPDATED, (_event, data) =>
        setAudioTracks(data.audioTracks as unknown as TrackOption[])
      );
      hls.on(Hls.Events.AUDIO_TRACK_SWITCHED, (_event, data) => setCurrentAudioTrack(data.id));
      hls.on(Hls.Events.SUBTITLE_TRACKS_UPDATED, (_event, data) =>
        setSubtitleTracks(data.subtitleTracks as unknown as TrackOption[])
      );
  ...
  const handleProgressPointerDown = (e: ReactPointerEvent) => {
    e.stopPropagation();
...
interface SettingsListProps<T extends string | number> {
  heading: string;
...
function SettingsList<T extends string | number>({
  heading,
  options,
  activeId,
  onSelect,
