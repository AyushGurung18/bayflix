...
export async function getRatings({ tmdbId, mediaType, imdbId, title, year }) {
  const params = new URLSearchParams({ tmdbId: String(tmdbId), mediaType });
  if (imdbId) params.set("imdbId", imdbId);
  if (title) params.set("title", title);
  if (year) params.set("year", year);
  return call(`/ratings?${params.toString()}`);
}

export async function getMyRatings() {
  const data = await call("/ratings/mine", { auth: true });
  return data?.results ?? [];
}
export async function rateTitle(item, mediaType, stars) {
...
export async function removeRating(tmdbId, mediaType) {
